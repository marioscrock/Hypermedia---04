	...
	$.when($.ajax({
		...
		, success: function (response) {
			...
			if (GET["item"] === "product_categories") {
				...
				for (var i = 0; i < response.length; i++) {
					tiles += "<div class=\x22col-xs-12 col-sm-6 col-md-4 col-lg-3 tileContainer same-height\x22>	<div class=\x22container-fluid tile\x22>	<img src=\x22" + response[i].img + "\x22 class=\x22img-responsive\x22 alt=\x22Generic placeholder thumbnail \x22>	<div class='btContainer'> <div class=\x22item\x22><p><b>" + response[i].name + "</b></p></div> <div class=\x22item \x22><button type =\x22button\x22 id=\x22btn-detail\x22 class=\x22btn btn-primary\x22 onclick=\x22location.href = 'servizio.html?id=" + response[i].id + "'\x22 > Dettagli </button></div> </div></div> </div>";
				}
			}
			$("#tiles").html(tiles);
