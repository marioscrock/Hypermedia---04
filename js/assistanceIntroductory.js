...
function ready() {
  ...
  $.when(
    ...
    , $.ajax({
      ...
      , dataType: "json", //type of data
      crossDomain: true, //localhost purposes
      url: "http://hyppolenghiscroccavendra.altervista.org/php/assistanceIntroductoryData.php", //Relative or absolute path to file.php file
      data: {
        id: GET["category"]
      }
      , success: function (response) {
        ...
        for (var i = 0; i < subcategories.length; i++) {
          subcategory += "<div class='col-xs-12 col-sm-6 col-md-6 col-lg-4 same-height'><ul class='custom-bullet'><h2>" + subcategories[i].toUpperCase() + "</h2>";
          for (var j = 0; j < response.length; j++) {
            if (response[j].name === subcategories[i]) {
              subcategory += "<li><a href='assistenza.html?id=" + response[j].id_ass + "'>" + response[j].title + "</a></li>";
            }
          }
          subcategory += "</ul></div>";
        }
