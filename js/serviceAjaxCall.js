  ...
  $.ajax({
    ...
    , success: function (response) {
      if (response[0].answer.length >= 0) {
        var button = "";
        button = "<div><button type =\x22button\x22 id=\x22btn-detail\x22 class=\x22btn btn-primary center-block\x22 style=\x22margin-top:20px; \x22 onclick=\x22location.href = 'FAQ.html?id=" + response[0].id_serv + "'\x22 > FAQ </button></div>";
      }
