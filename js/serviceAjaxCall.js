...
function ready() {
	...
	$.ajax({
		...
		, success: function (response) {
			...
			$("#btn-form").html("<button type =\x22button\x22 class=\x22btn btn-primary\x22 onclick=\x22location.href = 'form-servizi.html?id=" + response[0].id_serv + "'\x22 >Sottoscrivi Servizio<br> Maggiori Informazioni</button>");
		}
		, error: function (request, error) {
			console.log("Error");
		}
